...
5. 左右两栏均可双击文件夹和 `..`，或在地址栏输入目录后按 Enter。Windows 可直接输入 `D:\` 切换磁盘。
...
从**本地文件列表**拖拽一个或多个文件 / 文件夹到其他应用。采用 Qt 原生 `QDrag` 与 `QMimeData.setUrls()`，携带真实的 `file://` URL，仅允许复制动作，不移动原文件。

是否接受文件夹取决于目标应用和操作系统。例如目标应用仅接受普通文件时，请先自行压缩文件夹。当前自动测试验证了文件与文件夹 URL、中文和空格路径；微信 / 飞书实际接收及 Ubuntu Wayland 的桌面交互仍需在对应环境验收。
