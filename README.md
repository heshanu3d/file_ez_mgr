...
6. 点击 **名称 / 类别 / 类型 / 大小 / 修改时间** 列标题升降序排序。默认按修改时间降序排列，最新项目在前；`..` 始终置顶。大小按实际字节数排序。本地和远端列表的列宽均可拖动列标题边界调整；文件名显示不全时，拖动“名称”与“类别”之间的边界加宽名称列，超出视图宽度时可使用底部横向滚动条。
...
12. 文件夹传输先扫描文件数量与总大小，再在任务列表展示已处理文件数、整体进度、当前文件名、当前文件大小及已传输大小。可悬停查看完整路径，跳过的文件会计入已处理数，空文件夹也可正常完成。选中任务可取消，网络阻塞时会在 I/O 返回或超时后生效。

### 恢复上次窗口

拖动窗口边界调整大小或最大化时自动保存窗口大小与状态，并在下次启动恢复；最大化会另外记住正常窗口的尺寸，以便取消最大化后恢复。退出时还会保存打开的标签页、排列顺序、当前选中的页面和各标签的本地 / 远端目录；下次启动直接回到上次页面。关闭窗口和通过应用菜单退出均会保存，取消关闭则不更新记录。首次启动或没有可恢复的标签页时才打开“本地浏览”。

窗口记录保存在配置目录的 `workspace.json`，不保存密码或传输队列。使用 SSH 私钥的 SFTP 连接会自动尝试重连；需要密码的连接保留页面，点击“重新连接”后输入凭据。已删除的连接会跳过，本地目录不存在时回退到用户目录。

### 下载历史

“同名文件”左侧的 **下载历史** 按“本地父目录 ← 远端父目录”记录成功下载，每个连接保留最近 50 对目录。同一对目录下下载不同文件或文件夹均合并为一条记录，最近使用的目录排在最前。例如 `path_a/file_a ← remote_path_a/file_a` 和 `path_a/dir_b ← remote_path_a/dir_b` 都记为 `path_a ← remote_path_a`。选择记录即跳转两侧父目录，并清空文件名筛选。

顶部选择连接，打开 **设置 → 编辑下载历史…**，即可查看本地目录、远端目录和最近下载时间。支持多选后点击 **删除选中记录**；删除立即生效，不删除实际文件，也不因取消连接设置而撤销。删除时保留未选中的新下载记录，各连接的历史相互隔离。

历史保存在配置目录的 `download-history/` 中，重启后自动读取，同一连接的多个标签页展开下拉框时会刷新。旧版记录按已保存的目录对合并重复项；由于旧数据不含文件 / 文件夹标记，原目录位置保留，可通过设置手动删除。失败、取消以及全部跳过的下载不新增历史。损坏的记录文件会保留并显示错误。TFTP 无目录浏览能力，不记录下载历史。
